            ...
            if(playerRewardProgress.progresses.length == 0){
               ...
               $percent = 0;
                ...
                if ((playerRewardProgress.progresses[0].possible-playerRewardProgress.progresses[0].earned) == 0) {
                    $criteriaMark = '<img src="'+jive.app.url({path:'/plugins/gamification-plugin/resources/img/Checkmark.svg'})+'"/>';
...
templateModal = {
    ...
    init: function(data, target){
        var self = this;
        var url= jive.soy.func.normalizeUrl(window._jive_base_url, '/mission-modal.jspa');
        $.ajax(url, {
            type: 'POST',
            data: data,
